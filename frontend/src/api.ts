...
export type Proposal = { scene_name: string; trigger: { type: 'resident_arrives' }; conditions: { type: 'time_after'; value: string }[]; actions: Action[]; status: 'ready' | 'needs_clarification' | 'unsupported' | 'service_unavailable' | 'rejected'; explanation: string; reason?: string };
...
export const simulateArrival = () => request<{ executions: { scene_name: string; results: { device_id: string; status: string; detail: string }[] }[] }>('/simulation/resident-arrival', { method: 'POST', body: JSON.stringify({}) });
